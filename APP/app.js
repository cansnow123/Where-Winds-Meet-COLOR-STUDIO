...
const STORAGE_KEYS = {
  visualDraft: "yanyun-color-studio-mobile-draft"
};
...
function syncCurrentColorDisplay() {
  ...
  const colorIndicator = document.querySelector(".current-color i");
  if (colorIndicator) {
...
function renderPresetSwatches() {
  ...
  Object.entries(PRESET_COLORS).forEach(([code, color]) => {
    ...
    button.setAttribute("aria-pressed", String(activePresetCode === code));
    button.innerHTML = `
...
function copyText(value, successMessage) {
  if (!value) {
    setStatus("没有可复制的彩字内容。", true);
    return;
  }

  const fallback = () => {
    const copied = fallbackCopyText(value);
    setStatus(copied ? successMessage : "复制失败，请手动复制。", !copied);
  };
...
function scheduleDraftSave() {
  ...
  draftSaveTimer = window.setTimeout(() => saveDraft(true), 450);
}

function restoreDraft() {
  let saved = null;
  try {
    saved = localStorage.getItem(STORAGE_KEYS.visualDraft);
  } catch {
    setStatus("当前浏览器无法读取本地草稿，已载入示例。", true);
  }
  if (saved) {
...
function bindEditorEvents() {
  ...
  elements.richEditor.addEventListener("pointerup", updateSelectionHint);
  elements.richEditor.addEventListener("touchend", () => window.setTimeout(updateSelectionHint, 0), { passive: true });
...
function bindEvents() {
  ...
  elements.modeTabs.forEach((button, index) => {
    ...
    button.addEventListener("keydown", (event) => {
      ...
      event.preventDefault();
      const tabCount = elements.modeTabs.length;
      ...
      nextTab.focus();
      activateMode(nextTab.dataset.mode);
    });
  });
  ...
  document.querySelector("#clearEditorBtn").addEventListener("click", () => {
    if (getPlainTextFromEditor() && !window.confirm("确定清空编辑区中的全部文案吗？")) {
      return;
    }
    setEditorFromText("");
    updateFromEditor();
